...
from api.neural_style_transfer.utils import StyleTransferUtils
from api.neural_style_transfer.normalization import Normalization
from api.neural_style_transfer.loss_functions import ContentLossModule, StyleLossModule
    ...
    def get_model(self, content_image, style_image):
        ...
        for layer in StyleTransferUtils.models[self.settings.model_choice].children():
            ...
            if name in StyleTransferUtils.content_layers[self.settings.model_choice]:
                ...
                content_loss = ContentLossModule(content)
                content_index += 1
            ...
            if name in StyleTransferUtils.style_layers[self.settings.model_choice]:
                ...
                style_loss = StyleLossModule(style)
                style_index += 1
                model.add_module('style_loss_{}'.format(style_index), style_loss)
                self.style_losses.append(style_loss)

        for index in range(len(model) - 1, -1, -1):
            if isinstance(model[index], ContentLossModule) or isinstance(model[index], StyleLossModule):
                return model[:(index + 1)]

        return model

    def style_transfer(self, content_image, style_image, content_weight=1, style_weight=10000000):
